 ...
 * @param
 *       	 bool	internal variable to determine recursion status - do not use
 *        	in calls
 * @return array
 */
...
if (! function_exists ( 'get_dir_file_info' )) {
	function get_dir_file_info($source_dir, $top_level_only = TRUE, $_recursion = FALSE) {
		...
		if ($fp = @opendir ( $source_dir )) {
			...
			// foreach (scandir($source_dir, 1) as $file) // In addition to
			// being PHP5+, scandir() is simply not as fast
			while ( FALSE !== ($file = readdir ( $fp )) ) {
 ...
 * Second parameter allows you to explicitly declare what information you want
 * returned
 ...
 *
 * Note: this is NOT an accurate way of determining file mime types, and is here
 * strictly as a convenience
