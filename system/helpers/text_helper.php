...
if (! function_exists ( 'ascii_to_entities' )) {
	function ascii_to_entities($str) {
		...
		for($i = 0, $s = strlen ( $str ); $i < $s; $i ++) {
			...
			if ($ordinal < 128) {
				/*
				 * If the $temp array has a value but we have moved on, then it
				 * seems only fair that we output that entity and restart $temp
				 * before continuing. -Paul
				 */
				if (count ( $temp ) == 1) {
 ...
 * @param
 *       	 string	the array of censoered words
 * @param
...
if (! function_exists ( 'highlight_code' )) {
	function highlight_code($str) {
		...
		// Remove our artificially added PHP, and the syntax highlighting that
		// came with it
 ...
 *       	 string	the text string
 * @param
 *       	 string	the phrase you'd like to highlight
 * @param
 *       	 string	the openging tag to precede the phrase with
 * @param
 *       	 string	the closing tag to end the phrase with
 * @return string
 */
 ...
 *       	 string	the text string
 * @param
 *       	 integer	the number of characters to wrap at
 * @return string
 */
