...
class XML_RPC_Message extends CI_Xmlrpc {
	...
	function open_tag($the_parser, $name, $attrs) {
		...
		switch ($name) {
			...
			case 'BASE64' :
				...
				break;
			case 'MEMBER' :
				// Set name of <member> to nothing to prevent errors later if no
				// <name> is found
				$this->xh [$the_parser] ['valuestack'] [0] ['name'] = '';
				
				// Set NULL value to check to see if value passed for this
				// param/member
				$this->xh [$the_parser] ['value'] = NULL;
				break;
			case 'DATA' :
	...
	function closing_tag($the_parser, $name) {
		if ($this->xh [$the_parser] ['isf'] > 1)
			...
			// the opening and closing of elements. Nesting is checked on the
		// opening
		...
		switch ($name) {
			...
			case 'BASE64' :
				...
				if ($name == 'STRING') {
					...
					// we have a DOUBLE
					// we must check that only 0123456789-.<space> are
					// characters here
					...
					// we have an I4/INT
					// we must check that only 0123456789-<space> are characters
					// here
			...
			case 'VALUE' :
				...
				// build the XML-RPC value out of the data received, and
				// substitute it
			...
			case 'METHORESPONSE' :
				...
				break;
			default :
				// End of an Invalid Element. Taken care of during the opening
				// tag though
				break;
		}
	}
