...
class CI_Xmlrpcs extends CI_Xmlrpc {
	var $methods = array (); // array of methods mapped to function names and
	                         // signatures
	...
	function add_to_map($methodname, $function, $sig, $doc) {
		$this->methods [$methodname] = array ('function' => $function, 'signature' => $sig, 'docstring' => $doc );
	}
		...
		if ($objectCall === TRUE) {
			if ($method_parts [0] == "this" && $system_call == TRUE) {
				...
				if ($this->object === FALSE) {
					...
					// return
				// call_user_func(array(&$method_parts['0'],$method_parts['1']),
				// $m);
				}
			}
		} else {
	...
	function multicall($m) {
		...
		foreach ( $calls as $value ) {
			// $attempt = $this->_execute(new XML_RPC_Message($value[0],
			// $value[1]));
