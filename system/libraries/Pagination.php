...
class CI_Pagination {
	...
	var $num_links = 2; // Number of "digit" links to show before/after the
	                       // currently viewed page
	var $cur_page = 0; // The current page being viewed
	var $use_page_numbers = FALSE; // Use page number for segment instead of
	                               // offset
			...
			for($loop = $start - 1; $loop <= $end; $loop ++) {
				...
				if ($i >= $base_page) {
					if ($this->cur_page == $loop) {
						$output .= $this->cur_tag_open . $loop . $this->cur_tag_close; // Current
						                                                           // page
					} else {
						$n = ($i == $base_page) ? '' : $i;
