...
class CI_Form_validation {
	...
	public function set_rules($field, $label = '', $rules = '') {
		...
		// If an array was passed via the first parameter instead of indidual
		// string
		...
		// Is the field name an array? We test for the existence of a bracket
		// "[" in
		// the field name to determine this. If it is an array, we break it
		// apart
		// into its components so that we can fetch the corresponding POST data
		// later
	...
	public function run($group = '') {
		...
		foreach ( $this->_field_data as $field => $row ) {
			// Fetch the data from the corresponding $_POST array and cache it
			// in the _field_data array.
		...
		foreach ( $this->_field_data as $field => $row ) {
			if (! is_null ( $row ['postdata'] )) {
				if ($row ['is_array'] == FALSE) {
					...
					// before we assign values, make a reference to the right
					// POST key
					if (count ( $row ['keys'] ) == 1) {
