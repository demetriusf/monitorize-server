...
class CI_DB_active_record extends CI_DB_driver {
	...
	public function from($from) {
		foreach ( ( array ) $from as $val ) {
			if (strpos ( $val, ',' ) !== FALSE) {
				...
				// in the _protect_identifiers to know whether to add a table
				// prefix
	...
	protected function _where($key, $value = NULL, $type = 'AND ', $escape = NULL) {
		...
		// If the escape value was not set will will base it on the global
		// setting
		if (! is_bool ( $escape )) {
		...
		foreach ( $key as $k => $v ) {
			...
			if (is_null ( $v ) && ! $this->_has_operator ( $k )) {
				// value appears not to have been set, assign the test to IS
				// NULL
				$k .= ' IS NULL';
			}
	...
	protected function _like($field, $match = '', $type = 'AND ', $side = 'both', $not = '') {
		...
		foreach ( $field as $k => $v ) {
			...
			// some platforms require an escape sequence definition for LIKE
			// wildcards
	...
	public function insert_batch($table = '', $set = NULL) {
		...
		if (count ( $this->ar_set ) == 0) {
			if ($this->db_debug) {
				// No valid data array. Folds in cases where keys and values did
				// not match up
	...
	protected function _compile_select($select_override = FALSE) {
		...
		if ($select_override !== FALSE) {
			...
			if (count ( $this->ar_select ) == 0) {
				...
				// Cycle through the "select" portion of the query and prep each
				// column name.
				// The reason we protect identifiers here rather then in the
				// select() function
				// is because until the user calls the from() function we don't
				// know if there are aliases
	...
	public function _object_to_array($object) {
		...
		foreach ( get_object_vars ( $object ) as $key => $val ) {
			// There are some built in keys we need to ignore for this
			// conversion
	 ...
	 *
	 * Called by the insert() update() insert_batch() update_batch() and
	 * delete() functions
	 *
	 * @return void
	 */
