...
class CI_DB_cubrid_forge extends CI_DB_forge {
		...
		foreach ( $fields as $field => $attributes ) {
			...
			if (is_numeric ( $field )) {
				...
				if (array_key_exists ( 'UNSIGNED', $attributes ) && $attributes ['UNSIGNED'] === TRUE) {
					// $sql .= ' UNSIGNED';
					// As of version 8.4.0 CUBRID does not support UNSIGNED
				// INTEGER data type.
					// Will be supported in the next release as a part of MySQL
				// Compatibility.
				}
	...
	function _create_table($table, $fields, $primary_keys, $keys, $if_not_exists) {
		...
		if (count ( $primary_keys ) > 0) {
			$key_name = "pk_" . $table . "_" . $this->db->_protect_identifiers ( implode ( '_', $primary_keys ) );
