	...
	function _create_table($table, $fields, $primary_keys, $keys, $if_not_exists) {
		...
		if ($if_not_exists === TRUE) {
			if ($this->db->table_exists ( $table )) {
				return "SELECT * FROM $table"; // Needs to return innocous but valid
				                               // SQL statement
			}
		}
		...
		foreach ( $fields as $field => $attributes ) {
			...
			// numeric, we know it was assigned by PHP and the developer
			// manually
			...
			if (is_numeric ( $field )) {
				...
				// If this is an auto-incrementing primary key, use the serial
				// data type instead
				...
				// Added new attribute to create unqite fields. Also works with
				// MySQL
		...
		if (count ( $primary_keys ) > 0) {
			// Something seems to break when passing an array to
