...
class Key extends REST_Controller {
	...
	public function index_put() {
		...
		if (self::_insert_key ( $key, array ('level' => $level, 'ignore_limits' => $ignore_limits ) )) {
			$this->response ( array ('status' => 1, 'key' => $key ), 201 ); // 201 =
			                                                           // Created
		} 

		else {
			$this->response ( array ('status' => 0, 'error' => 'Could not save the key.' ), 500 ); // 500
			                                                                                  // =
			                                                                                  // Internal
			                                                                                  // Server
			                                                                                  // Error
	...
	public function level_post() {
		...
		if (self::_update_key ( $key, array ('level' => $new_level ) )) {
			$this->response ( array ('status' => 1, 'success' => 'API Key was updated.' ), 200 ); // 200
			                                                                                 // =
			                                                                                 // OK
		} 

		else {
			$this->response ( array ('status' => 0, 'error' => 'Could not update the key level.' ), 500 ); // 500
			                                                                                          // =
			                                                                                          // Internal
			                                                                                          // Server
			                                                                                          // Error
	...
	public function suspend_post() {
		...
		if (self::_update_key ( $key, array ('level' => 0 ) )) {
			$this->response ( array ('status' => 1, 'success' => 'Key was suspended.' ), 200 ); // 200
			                                                                               // =
			                                                                               // OK
		} 

		else {
			$this->response ( array ('status' => 0, 'error' => 'Could not suspend the user.' ), 500 ); // 500
			                                                                                      // =
			                                                                                      // Internal
			                                                                                      // Server
			                                                                                      // Error
	...
	public function regenerate_post() {
		...
		if (self::_insert_key ( $new_key, array ('level' => $key_details->level, 'ignore_limits' => $key_details->ignore_limits ) )) {
			...
			$this->response ( array ('status' => 1, 'key' => $new_key ), 201 ); // 201 =
			                                                               // Created
		} 
