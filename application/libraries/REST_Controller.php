...
abstract class REST_Controller extends CI_Controller {
	 ...
	 * @var array
	 */
	protected $_supported_formats = array ('xml' => 'application/xml', 'json' => 'application/json', 'jsonp' => 'application/javascript', 'serialized' => 'application/vnd.php.serialized', 'php' => 'text/plain', 'html' => 'text/html', 'csv' => 'application/csv' );
	...
	public function __construct() {
		...
		// This library is bundled with REST_Controller 2.5+, but will
		// eventually be part of CodeIgniter itself
		...
		// Now we know all about our request, let's try and parse the body if it
		// exists
		...
		// When there is no specific override for the current class/method, use
		// the default auth value set in the config
	...
	public function response($data = array(), $http_code = null) {
		...
		if (empty ( $data ) && $http_code === null) {
			...
			// create the output variable here in the case of
			// $this->response(array());
			$output = NULL;
		} 		
		...
		// camping!
			...
			// If the format method exists, call and return the output in that
			// format
	...
	protected function _detect_output_format() {
		...
		// A format has been passed as an argument in the URL and it is
		// supported
		...
		if ($this->config->item ( 'rest_ignore_http_accept' ) === FALSE and $this->input->server ( 'HTTP_ACCEPT' )) {
			...
			foreach ( array_keys ( $this->_supported_formats ) as $format ) {
				...
				if (strpos ( $this->input->server ( 'HTTP_ACCEPT' ), $format ) !== FALSE) {
					// If not HTML or XML assume its right and send it on its
					// way
		...
		// Well, none of that has worked! Let's see if the controller has a
		  // default
	...
	protected function _detect_api_key() {
		...
		if (($key = isset ( $this->_args [$api_key_variable] ) ? $this->_args [$api_key_variable] : $this->input->server ( $key_name ))) {
			...
			/*
			 * If "is private key" is enabled, compare the ip address with the
			 * list of valid ip addresses stored in the database.
			 */
				...
				if (isset ( $row->ip_addresses )) {
					...
					foreach ( $list_ip_addresses as $ip_address ) {
						if ($this->input->ip_address () == trim ( $ip_address )) {
							// there is a match, set the the value to true and
					...
					// There should be at least one IP address for this private
					// key.
					return FALSE;
				}
			}
	...
	protected function _log_request($authorized = FALSE) {
		return $this->rest->db->insert ( config_item ( 'rest_logs_table' ), array ('uri' => $this->uri->uri_string (), 'method' => $this->request->method, 'params' => $this->_args ? serialize ( $this->_args ) : null, 'api_key' => isset ( $this->rest->key ) ? $this->rest->key : '', 'ip_address' => $this->input->ip_address (), 'time' => function_exists ( 'now' ) ? now () : time (), 'authorized' => $authorized ) );
	}
	...
	protected function _check_limit($controller_method) {
		...
		$result = $this->rest->db->where ( 'uri', $this->uri->uri_string () )->where ( 'api_key', $this->rest->key )->get ( config_item ( 'rest_limits_table' ) )->row ();
		
		// No calls yet, or been an hour since they called
		if (! $result or $result->hour_started < time () - (60 * 60)) {
			// Right, set one up from scratch
			$this->rest->db->insert ( config_item ( 'rest_limits_table' ), array ('uri' => $this->uri->uri_string (), 'api_key' => isset ( $this->rest->key ) ? $this->rest->key : '', 'count' => 1, 'hour_started' => time () ) );
		} 		
		...
		// They have called within the hour, so lets update
			...
			$this->rest->db->where ( 'uri', $this->uri->uri_string () )->where ( 'api_key', $this->rest->key )->set ( 'count', 'count + 1', FALSE )->update ( config_item ( 'rest_limits_table' ) );
		}
	...
	protected function _auth_override_check() {
		...
		// Check to see if the override array is even populated, otherwise
		// return false
		if (empty ( $this->overrides_array )) {
			return false;
		}
		
		// Check to see if there's an override value set for the current
		// class/method being called
		if (empty ( $this->overrides_array [$this->router->class] [$this->router->method] )) {
			return false;
		}
		
		// None auth override found, prepare nothing but send back a true
		// override flag
		...
		// Whitelist auth override found, check client's ip against config
		// whitelist
		...
		// Return false when there is an override value set but it does not
		// match
	 ...
	 *
	 * @param $key string
	 *       	 The key for the PUT request argument to retrieve
	 * @param $xss_clean boolean
	 *       	 Whether the value should be XSS cleaned or not.
	 ...
	 *
	 * @param $key string
	 *       	 The key for the DELETE request argument to retrieve
	 * @param $xss_clean boolean
	 *       	 Whether the value should be XSS cleaned or not.
	...
	protected function _prepare_digest_auth() {
		...
		$uniqid = uniqid ( "" ); // Empty argument for backward compatibility
		                      // We need to test which server authentication
		                      // variable to use
		                      // because the PHP ISAPI module in IIS acts
		                      // different from CGI
		...
		// We need to retrieve authentication informations from the $auth_data
		// variable
	...
	// FORMATING FUNCTIONS
	// ---------------------------------------------------------
	// Many of these have been moved to the Format class for better separation,
	// but these methods will be checked too
