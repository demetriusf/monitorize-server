...
class Spark_CLI {
	...
	private static $commands = array ('help' => 'help', 'install' => 'install', 'list' => 'lister', 'reinstall' => 'reinstall', 'remove' => 'remove', 'search' => 'search', 'sources' => 'sources', 'upgrade-system' => 'upgrade_system', 'version' => 'version', '' => 'index' )	// default action
	;
	...
	private function search($args) {
		...
		foreach ( $this->spark_sources as $source ) {
			...
			foreach ( $results as $result ) {
				...
				// only show the source information if there are multiple
				// sources
				if (count ( $this->spark_sources ) > 1)
					$result_line .= " (source: $source->url)";
