...
class Spark_source {
	...
	function get_spark_detail($spark_name, $version = 'HEAD') {
		...
		if (! $json_data)
			return null; // no such spark here
	...
	function search($term) {
		...
		$data = json_decode ( $json_data );
		// if the data isn't around of success is false, return a warning for
		// this source
		if ($data == null || ! $data->success) {
	...
	function outdated() {
		...
		// Compare
		if ($self_major < $source_major || $self_major == $source_major && $self_minor < $source_minor || $self_major == $source_major && $self_minor == $source_minor && $self_patch < $source_patch) {
	...
	private function get_spark($data) {
		...
		else if ($data->repository_type == 'git')
			return Git_spark::get_spark ( $data );
		else if ($data->repository_type == 'zip')
